...
composite score 0–100 that balances:
  ...
  - Premium / annualised return          (weight: 17%)
  - Delta / assignment risk              (weight: 22%)
  - Liquidity (OI + bid-ask spread)      (weight: 18%)
  - Chart alignment (near support)       (weight: 18%)
  - Desired buy price proximity          (weight: 13%)
  - IV rank                              (weight: 12%)
...
def _premium_score(annualised_return: float, risk_profile: RiskProfile) -> float:
    """Map annualised return → score via soft-cap logistic curve."""
    targets = {
